...
Create a macOS Big Sur style icon with Ghostty's blue-purple theme:

```bash
# Create squircle background with Ghostty blue-purple
...
# Generate iconset
mkdir -p ccOpener-ghostty.iconset
for size in 16 32 128 256 512; do
  magick icon_512.png -resize ${size}x${size} ccOpener-ghostty.iconset/icon_${size}x${size}.png
  magick icon_512.png -resize $((size*2))x$((size*2)) ccOpener-ghostty.iconset/icon_${size}x${size}@2x.png
done

# Convert to icns
iconutil -c icns ccOpener-ghostty.iconset -o ccOpener-ghostty.icns
```

Apply icon:
1. Copy `ccOpener-ghostty.icns` to `ccOpener-ghostty.app/Contents/Resources/AppIcon.icns`
2. Edit `ccOpener-ghostty.app/Contents/Info.plist`: set `CFBundleIconFile` to `AppIcon`

### 3. Add to Finder Toolbar

1. Open Finder
2. Hold **Command**
3. Drag `ccOpener-ghostty.app` to the toolbar
